...
MVOeditorShow.prototype.parse = function(input) {
    ...
    let objectModel = result['model MVO uses BaseModel'][''];
    var output = [];
    JSVisitor.travelOne(objectModel, null, function(keyArg, node, context) {
        if (keyArg === null) {
            ...
            if ((keyArg.startsWith('variable ') || tupleDefiniton)) {
                ...
                if (formula) {
                    var spaces = [];
                    let total = getdepth(node, 0) + " " + nodeName + " " + (locked ? 'locked ' : ' ') + (visible ? 'visible ' : ' ') + (required ? 'required ' : ' ');
                    spaces.length = 70 - total.length;
                    output.push(total + spaces.join(' ') + '=' + node.formula);
