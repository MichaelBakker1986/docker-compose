...
const wbKSP = new WorkBook(new Context())
wbKSP.importSolution(readFileSync(__dirname + '/KSP.ffl', 'utf8'), 'ffl')
...
FormulaService.visitFormulas(function(formula) {
	...
	if (!testVariables[variableName]) {
		if (!testedformulas[formula.original]) {
			untestedformulas++
			if (TRACE) trace('[%s][%s][%s][%s]', variableName.replace(/(^KSP_)/gmi, ''), formula.name, formula.original, formula.parsed)
		}
	}
})
if (DEBUG) debug('KSP untested formulas:[%s/%s]', untestedformulas, totalformulas)
wbKSP.set('FES_LAYOUT', 'IIFRS-PL')
const layoutNR = wbKSP.get('FES_LAYOUTNR')
const layoutNRChoice = wbKSP.get('FES_LAYOUTNR', 'choices')
equal(layoutNR, 'Polish')
equal(wbKSP.get('Q_RESTRICTIES_01'), '')
equal(wbKSP.get('CombinationDiscountPercentage'), .062)
equal(wbKSP.get('DecreasingPercentage'), 0.07)
equal(wbKSP.get('Age'), 0)
equal(wbKSP.get('Age', 'value', 4), 4)
equal(wbKSP.get('TestT'), 1)
equal(wbKSP.get('Furniture'), 1800)
equal(wbKSP.get('Furniture', 'value', 4), 0)
equal(wbKSP.get('DEBUG'), 0)
wbKSP.set('Memo1', 'a_Negro_a')
assert((Pos('Negro', 'a_Negro_a') > 0 ? 1 : 0), 1)
equal(wbKSP.get('Q_RESTRICTIES'), '')
equal(wbKSP.get('Q_RESTRICTIES_01'), '')
equal(wbKSP.get('Q_RESTRICTIESTXT'), '')
equal(wbKSP.get('Q_WARNING_01'), '')
equal(wbKSP.get('Q_WARNING_GLOBALTXT'), '')
equal(wbKSP.get('Q_MAP01'), 'Incomplete')
equal(wbKSP.get('Q_ROOT'), 'Incomplete')
equal(wbKSP.get('Q_WARNING_GLOBAL'), '')
equal(wbKSP.get('Q_RESULT'), 'Deze vragenlijst is definitief gemaakt.[br][/br]')
equal(wbKSP.get('Q_MAP06', 'visible'), true)
equal(wbKSP.get('Q_MAP06'), 'Incomplete')
let pad = '            '
wbKSP.get('CostsYearFiveSixSeven', 'value', 12)

function testVariable(variableName, level, column) {
	const indent = pad.substring(0, level)
	const result = {}
	const formula = formulas[variableName]
	const values = []
	if (TRACE) trace(`${values + indent}[%s][%s]=[%s]`, variableName, wbKSP.get(variableName, 'value', column), formula.original)
	for (let dependencyname in formula.deps) {
		const modelVarName = modelVariableName(dependencyname)
		if (variableName !== modelVarName) testVariable(modelVarName, level + 1, column)
	}
}
