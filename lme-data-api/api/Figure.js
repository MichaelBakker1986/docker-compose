...
const Figure = new Promise((accept, reject) => {
	orm.connectAsync(dbConnectString).then(async (db) => {
			...
			methods: {
				getScenarioFigures: function(ids) {
					return Promise.all([new Promise(function(ok, fail) {
						const sql = 'SELECT figure.* FROM figure join ( SELECT max(figure.id) as m from figure  where uuid IN (\'' + ids.join('\',\'') + '\') group by var,col ) as best on best.m=figure.id'
						...
						const sql = `SELECT * from figure_commit where uuid IN ('${ids.join('\',\'')}') order by id;`
						db.driver.execQuery(sql, [], function(err, result) {
							if (err) return fail(err)
							ok(result)
						})
				...
				getFigures        : function(id) {
					return Promise.all([new Promise(function(ok, fail) {
						const sql = 'SELECT figure.* FROM figure join ( SELECT max(figure.id) as m from figure inner join figure_tree on uuid_parent=figure.uuid where figure_tree.uuid=? group by var,col ) as best on best.m=figure.id'
						...
						//var sql = "SELECT uuid_parent from figure_tree where uuid=? order by id";
						const sql = 'SELECT fc.* from figure_tree as ft join figure_commit as fc on uuid_parent=fc.uuid where ft.uuid=? order by fc.id;'
						db.driver.execQuery(sql, [id], function(err, result) {
							if (err) return fail(err)
							ok(result)
						})
				...
				insertFigures     : function(parent, newChildId, values, saveTime) {
					return Promise.all([new Promise(function(ok, fail) {
						const sql = 'INSERT INTO figure_tree (uuid,uuid_parent)  SELECT \'' + newChildId + '\' as uuid,uuid_parent as uuid_parent FROM figure_tree where uuid = \'' + parent + '\' UNION  select \'' + newChildId + '\',\'' + newChildId + '\';'
						db.driver.execQuery(sql, [], function(err, result) {
							if (err) fail(err)
							ok(result)
						})
		...
		const FigureTree = db.define('figure_tree', {
			uuid       : String,
			uuid_parent: String
		}, {
			methods: {}
		...
		const FigureCommit = db.define('figure_commit', {
			uuid       : String,
