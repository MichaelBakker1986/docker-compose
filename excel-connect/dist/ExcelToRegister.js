'use strict';

var _typeof2 = require('babel-runtime/helpers/typeof');

var _typeof3 = _interopRequireDefault(_typeof2);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

var Excel = require('exceljs'),
    Promise = require('promise'),
    Stream = require('stream'),
    log = require('log6'),
    moment = require('moment'),
    Register = require('../lme-core/exchange_modules/ffl/Register').Register,
    RegisterToFFL = require('../lme-core/exchange_modules/ffl/RegisterToFFL').RegisterToFFL;

var variable_name_index = 3;
var excel_title_index = 1;
var depth_index = 2;
var required_index = 6;
var visible_index = 7;
var displaytype_index = 8;

var map = function map(value) {
    if (value) {
        if (value.text || value.hyperlink) return value.hyperlink || value.text || '';
        if (value.formula || value.result) return value.result || '';
        if (value instanceof Date) return dateFormat ? moment(value).format(dateFormat) : moment(value).format();
        if (value.error) return value.error;
        if ((typeof value === 'undefined' ? 'undefined' : typeof value === 'undefined' ? 'undefined' : (0, _typeof3.default)(value)) === 'object') {
            return JSON.stringify(value);
        }
    }
    return value;
};

function ExcelToFFLStream(stream, opts) {
    var audit = [];
    return new Promise(function (succes, fail) {
        new Excel.Workbook().xlsx.readFile(stream).then(function (data) {
            succes(data);
        }).catch(function (err) {
            console.info(err);
            fail('Error reading XLSX ' + JSON.stringify(opts), err.toString());
        });
    }).then(function (data) {
        var register = new Register();
        /*const csv = data.csv.writeFile('./raw_excel_data.csv', { delimiter: ";" })*/

        register.addColumn('title');
        register.addColumn('display_options');
        register.addColumn('required');
        register.addColumn('visible');
        register.addColumn('displaytype');
        var valid_excel_row = function valid_excel_row(row) {
            if (row.length <= 2) return false;
            var variable_name = row[variable_name_index];
            if (!variable_name) return false;
            variable_name = variable_name.replace(/ /gmi, '');
            return variable_name.length > 2;
        };
        register.addRowSave([0, 1, '', opts.root_name, 0, null, null, false, false, false, [], null, '"Presentation root variable"', 'scorecard', false, null, null]);
        var worksheet = data.getWorksheet(opts.presentation_sheet_name);
        var children_index = register.schemaIndexes.children;
        var parent_index = register.schemaIndexes.parentId;

        var matrix = [];
        if (worksheet) {
            worksheet.eachRow(function (row, rowNumber) {
                var values = row.values;
                values.shift();
                if (rowNumber == 1) audit.push('Header columns: ' + values.map(map).join('|'));else {
                    var vals = values.map(map);
                    if (valid_excel_row(vals)) matrix.push(vals);else audit.push('Invalid row ' + rowNumber + ": " + vals.join('|'));
                }
            });
        }

        var parents = [];
        var curr_depth = 1;
        for (var i = 0; i < matrix.length; i++) {
            var excel_row = matrix[i];

            var variable_name = "P" + i + "_" + excel_row[variable_name_index].replace(/ /gmi, '');
            var refer_variable_name = excel_row[variable_name_index].replace(/ /gmi, '');
            var current_depth = excel_row[depth_index];
            if (current_depth != curr_depth) {
                if (current_depth > curr_depth) {
                    if (current_depth != curr_depth + 1) {
                        //log.info('r:' + i + ": " + curr_depth + " to " + current_depth)
                        audit.push('Invalid row ' + 'r:' + i + ": " + curr_depth + " to " + current_depth + excel_row.join('|'));
                    }
                }
                curr_depth = current_depth;
            }
            parents[current_depth] = variable_name;
            var parent_name = parents[current_depth - 1];
            var required = parseInt(excel_row[required_index]) == 1 ? 'On' : null;
            var visible = parseInt(excel_row[visible_index]) == 0 ? 'Off' : null;
            var display_type = excel_row[displaytype_index] || null;

            register.addRowSave([0, 1, '', variable_name, i + 1, null, parent_name, false, refer_variable_name, false, [], null, '"' + excel_row[excel_title_index] + '"', null, required, visible, display_type]);
        }
        var names = register.getIndex('name');
        var root = names[opts.root_name];
        for (var i = 1; i < register.i.length; i++) {
            var n = register.i[i];
            var pp = names[n[parent_index]] || root;
            pp[children_index].push(n);
        }
        var sizes = register.i[0].map(function (el) {
            return 10;
        });
        //console.info(register.getAll('refersto').join('\n'))
        var child_index = register.schemaIndexes.children;
        var filter = function filter(f, i) {
            return i > 2 && i != child_index;
        };

        var registerToFFL = new RegisterToFFL(register);
        return {
            ffl: registerToFFL.toGeneratedFFL(opts.root_name, null),
            csv: registerToFFL.toCSV(opts.root_name),
            register: register.getAll('refersto').splice(1),
            names: register.getAll('names'),
            audit: audit
        };
    }).catch(function (err) {
        log.error(err);
        audit.push(err.toString());
        return { ffl: '', register: '', audit: audit };
    });
}

/*
 ExcelToFFLStream(__dirname + '/resources/VASTGOED_def2.xlsx', {
    root_name              : 'Q_ROOT',
    name                   : 'VASTGOED',
    presentation_sheet_name: 'Presentation',
    columns                : ["rownumber", "description", "level", "name", "*", "*", "required", "visible", 'displaytype']
}).then(result => {

    console.info(result.ffl.join('\n'))
}).catch((err) => {
    console.error(err.toString())
})
*/

exports.parse = ExcelToFFLStream;
module.exports = ExcelToFFLStream;