...
import { RegisterToFFL } from '../ffl/index'
...
function ExcelToFFLStream(stream, opts) {
	...
	return new Promise(function(success, fail) {
		...
		if (worksheet) {
			worksheet.eachRow((row, rowNumber) => {
				...
				if (rowNumber === 1) audit.push('Header columns: ' + values.map(map).join('|'))
					...
					const vals = values.map(map)
					if (valid_excel_row(vals)) matrix.push(vals)
					else audit.push('Invalid row ' + rowNumber + ': ' + vals.join('|'))
				}
			})
		}
		...
		const parents = []
		let curr_depth = 1
		for (let matrix_index = 0; matrix_index < matrix.length; matrix_index++) {
			const excel_row = matrix[matrix_index]

			const variable_name = `P${matrix_index}_${excel_row[variable_name_index].replace(/ /gmi, '')}`
			...
			const display_type = excel_row[displaytype_index] || null
