...
class EconomicEditorView {
	constructor() {
		this.on = false
		this.properties = true
	}

	parse(input, rootNodeName) {
		const indexer = new Register()
		const fflFormatter = new FFLToRegister(indexer, input)
		...
		indexer.walk(rootNode, 0, function(node, depth) {
			...
			let formula = node[notrend_formulaIndex] || node[formulaIndex]
