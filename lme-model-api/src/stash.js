...
.then(() => log.debug('Added "git ls-files"'))
.catch(() => log.error('Cannot symlink "git ls-files"'))
...
class Stash {
	...
	preview(user_id, name, data) {
		...
		.then(() => {
			return exec(`node -r babel-register ${__dirname}/exportLME_FFL.js ${tempHash}`).then((result) => {
	...
	commit(user_id, name, data, type) {
		...
		.then(function(filename) {
			return Promise.all([exec(`node -r babel-register ${__dirname}/exportLME_FFL.js ${name} ${type === '.ffl' ? 'FFL2' : 'FFL'}`)]).then((result) => {
