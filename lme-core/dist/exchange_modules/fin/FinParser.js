...
var parser = {
	...
	parse: function parse(data) {
		...
		//.with a .valid
		var solution;
		var parseResult = _FinToJSON2.default.parse(data);
	...
	deParse: function deParse(rowId) {
		var finExport = _PropertiesAssembler2.default.create();
		var exportString = '';
		//= finExport.getName() + "\n\n";
		...
		_PropertiesAssembler2.default.visit(startuielem, function (node, depth) {
			var formulaProperties = finExport.gatherFormulaProperties(_ValueFacade2.default.getFormula, _ValueFacade2.default.properties, node.rowId);
...
function createVariableTree(variables, solution) {
	...
	var cache = {};
	var stack = new _stackAdt2.default();
...
function createFinFormulaRow(key, variable, formula) {
	...
	var pad = '                                      ';
	var and = str + pad.substring(0, pad.length - str.length);
	var deparsed = _FinFormula2.default.javaScriptToFinGeneric(formula);
