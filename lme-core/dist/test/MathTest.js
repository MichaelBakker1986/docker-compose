...
(0, _assert.ok)(wb.get('CaseTestVariableWithReference'), 223);

wb.createFormula('1+1', 'A', 'A');
(0, _assert.equal)(wb.get('A', 'A'), 2);
...
(0, _assert.equal)(wb.get('ROUND'), 12.1);
(0, _assert.equal)(wb.get('ROUND2'), 13);

/**
 * test OnER function
 */
wb.createFormula('OnER("ab",2)', 'ONERRCALL');
(0, _assert.equal)(wb.get('ONERRCALL'), 2);

/**
 * test Pos function
 */
wb.createFormula('Pos(\'cd\',\'abcd\')', 'POS');
(0, _assert.equal)(wb.get('POS'), 2);
...
(0, _assert.equal)(wb.get('DATAAVAILABLE'), false);
wb.set('abc12', 'anyValue');
(0, _assert.equal)(wb.get('DATAAVAILABLE'), true);

wb.createFormula('0', 'caseselectWithColon');
var caseTest = 'Case(caseselectWithColon,[0, 576 || 1, 906 || 2, 535 || 3, 535])';
wb.createFormula(caseTest, 'caseselectWithColonVariable');
(0, _assert.ok)(wb.get('caseselectWithColonVariable'), 576);

wb.createFormula('100+Tsy', 'TSY_TEST');
(0, _assert.equal)(wb.get('TSY_TEST'), 101);
...
var fesGetValue = _index2.default.getValue({
  properties: {
    choices: true,
    value: true
  },
...
(0, _assert.equal)(wb.get('ONZERO_TEST'), 2);
wb.createFormula('OnZero(NA,2)', 'ONZERO_TESTNA');
(0, _assert.equal)(wb.get('ONZERO_TESTNA'), NA);

wb.createFormula('MinMax(100,110,200,NA)', 'MINMAX_VALUEBOTTOM');
(0, _assert.equal)(wb.get('MINMAX_VALUEBOTTOM'), 110);
wb.createFormula('MinMax(300,110,200,NA)', 'MINMAX_VALUETOP');
(0, _assert.equal)(wb.get('MINMAX_VALUETOP'), 200);
wb.createFormula('MinMax(\'a\',110,200,300)', 'MINMAX_VALUE_FALLBACK');
(0, _assert.equal)(wb.get('MINMAX_VALUE_FALLBACK'), 300);

wb.createFormula('DMYtoDate(10,11,2020)', 'DMYTest');
var daysInMillis = 1000 * 3600 * 24;
(0, _assert.equal)(Math.round(wb.get('DMYTest').getTime() / daysInMillis), Math.round(new Date(2020, 10, 10).getTime() / daysInMillis));

/**
 * Lets make sure NA stays NA when performing arithmetic actions
 */
(0, _assert.equal)(OnNA(NA, 'NA'), 'NA');
(0, _assert.equal)(OnNA(-NA, 'NA'), 'NA');
(0, _assert.equal)(OnNA(100 * -NA, 'NA'), 'NA');
(0, _assert.equal)(OnNA(-100 * -NA, 'NA'), 'NA');
(0, _assert.equal)(OnNA(NA * NA * 9, 'NA'), 'NA');
(0, _assert.equal)(OnNA(NA / -2, 'NA'), 'NA');
(0, _assert.equal)(OnNA(NA + NA, 'NA'), 'NA');

wb.createFormula('Exp(5)', 'EXPTEST');
(0, _assert.equal)(wb.get('EXPTEST'), Math.exp(5));
