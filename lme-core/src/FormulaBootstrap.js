...
import simplified     from './ASTPreparser'
...
simplified.DataAvailable = (formulaInfo, node) => {
	...
	if (refFormula.ref === undefined) {
		return log.warn(`Can't find a variableReference for ${regenerate(node)} ${formulaInfo.name}:${formulaInfo.original}`)
	}
...
simplified.AnyDataAvailable = (formulaInfo, node) => {
...
simplified.TitleEntered = (formulaInfo, node) => {
...
FormulaBootstrap.prototype.parseAsFormula = function(formulaInfo) {
	ok(formulaInfo.parsed === undefined)
	var ast
...
export default FormulaBootstrap.prototype